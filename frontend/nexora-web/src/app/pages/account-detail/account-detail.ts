...
import { TransactionService, TransferResponse } from '../../services/transaction';
...
export class AccountDetail implements OnInit {
  ...
  getTransactionDescription(transaction: TransferResponse): string {
    if (transaction.toAccountId === this.accountId) {
      return `Transfer from ${this.getAccountName(transaction.fromAccountId)}`;
    }

    return `Transfer to ${this.getAccountName(transaction.toAccountId)}`;
  }
  getAccountName(accountId: number): string {
    const account = this.accounts.find((account) => account.id === accountId);

    return account?.type ?? `Account ${accountId}`;
  }
}
